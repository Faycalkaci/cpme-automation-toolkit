...
const TemplateFieldsPreview: React.FC<TemplateFieldsPreviewProps> = ({
  ...
  return (
    <div className="mt-4 p-4 bg-slate-50 rounded-md border border-slate-200 shadow-subtle">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
        {fieldsToDisplay.map((field, index) => (
          <div 
            key={index} 
            className="text-xs bg-white p-2 rounded border border-slate-100 flex items-center hover:border-primary/30 hover:shadow-subtle transition-all duration-200"
          >
            <span className="bg-primary/10 text-primary text-[10px] px-1.5 py-0.5 rounded mr-1.5 font-mono">{`{{${field}}}`}</span>
